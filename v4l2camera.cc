...
namespace {
    ...
    private:
        ...
        static void Watch(const Nan::FunctionCallbackInfo<v8::Value> &info, uv_poll_cb cb);
    ...
    void Camera::WatchCB(uv_poll_t *handle, void (*callbackCall)(CallbackData *data)) {
        ...
        uv_poll_stop(handle);
        uv_close(reinterpret_cast<uv_handle_t *>(handle), [](uv_handle_t *handle) -> void { delete handle; });
    ...
    NAN_METHOD(Camera::New) {
        ...
        self->device = *Nan::Utf8String(device);
    ...
    NAN_METHOD(Camera::Start) {
        ...
        const auto device = Nan::ObjectWrap::Unwrap<Camera>(info.Holder())->device;
        auto camera = camera_open(device.c_str());
        if (!camera) {
        ...
        Nan::ObjectWrap::Unwrap<Camera>(thisObj)->camera = camera;
